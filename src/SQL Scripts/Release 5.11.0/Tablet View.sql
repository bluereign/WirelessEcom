...
/****** Object:  View [catalog].[dn_Tablets]    Script Date: 05/12/2014 13:34:36 ******/
SET ANSI_NULLS ON
GO
...
Title (DeviceGuid, pageTitle, summaryTitle, detailTitle, sd, ld, mk, md, IsAvailableInWarehouse, IsAvailableOnline, bfreebit) AS 
		...
		INNER JOIN catalog.property AS p4 WITH (NOLOCK) ON p4.ProductGuid = d.TabletGuid AND p4.Name = 'MetaKeywords'
			AND p4.ProductGuid = p.ProductGuid 
			AND p4.ProductGuid = p2.ProductGuid AND p4.ProductGuid = p3.ProductGuid
		INNER JOIN catalog.property AS p5 WITH (NOLOCK) ON p5.ProductGuid = d.TabletGuid 
...
SELECT
	...
	, '<span class="rebate-callout">%CarrierRebate1% %CarrierSkuRebate1%</span>' + ISNULL (t.sd, NULL)	AS summaryDescription
	, '<span class="rebate-callout">%CarrierRebate1% %CarrierRebate2% %CarrierSkuRebate1% %CarrierSkuRebate2%</span>' + ISNULL (t.ld, NULL)	AS detailDescription
	, t.mk  + ', ' + STUFF(
             (SELECT ', ' + cptg.Tag
...
/****** Object:  View [catalog].[dn_AllProducts]    Script Date: 05/30/2014 09:12:04 ******/
SET ANSI_NULLS ON
GO
