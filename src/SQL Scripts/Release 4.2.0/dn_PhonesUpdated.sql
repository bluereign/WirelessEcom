...
RebateTot (ProGuid, N, A, U) AS
		(
			SELECT p.ProductGuid, vprt.RebateTotal, vprta.RebateTotal, vprtu.RebateTotal
			FROM catalog.product p
			INNER JOIN catalog.vProductRebateTotal AS vprt on vprt.ProductGuid = p.ProductGuid AND vprt.RebateMode = 'N'
			INNER JOIN catalog.vProductRebateTotal AS vprta on vprta.ProductGuid = p.ProductGuid AND vprta.RebateMode = 'A'
			AND vprta.ProductGuid = vprt.ProductGuid
			INNER JOIN catalog.vProductRebateTotal AS vprtu on vprtu.ProductGuid = p.ProductGuid AND vprtu.RebateMode = 'U'
			AND vprtu.ProductGuid = vprt.ProductGuid AND vprtu.ProductGuid = vprta.ProductGuid
...
SELECT
	...
	, ISNULL
                          ...
                          ((SELECT     RebateTotal
                              FROM         catalog.vProductRebateTotal AS vprt
                              WHERE     (ProductGuid = p.ProductGuid) AND (RebateMode = 'A')), CAST(0 AS money)) AS addaline_rebateTotal,
	d .UPC,
...
LEFT OUTER JOIN
	catalog.Product AS p ON pg.ProductGuid = p.ProductGuid
...
LEFT OUTER JOIN
	RebateTot AS rt ON rt.ProGuid = p.ProductGuid
