USE [STAGE.WIRELESSADVOCATES.COM]
GO

/****** Object:  View [catalog].[dn_Plans]    Script Date: 01/24/2013 13:41:45 ******/
SET ANSI_NULLS ON
GO
...
SELECT     r.RateplanGuid, p.ProductGuid, p.ProductId AS planId, p.ProductId, p.GersSku, r.CarrierBillCode, ISNULL
                      ...
                      AS additional_data_usage, CAST(ISNULL
                          ((SELECT     TOP (1) Value
                              FROM         catalog.Property AS Property_1
                              WHERE     (Name = 'DataLimitGB') AND (Value <> '') AND (ProductGuid = r.RateplanGuid) AND (Active = 1)), 0) AS VARCHAR(50)) 
                      AS DataLimitGB
                      , (SELECT TOP 1 py.Value FROM catalog.Property py WHERE Name = 'MetaKeywords' AND py.ProductGuid = p.ProductGuid) + ', ' + STUFF(
             (SELECT ', ' + cptg.Tag
