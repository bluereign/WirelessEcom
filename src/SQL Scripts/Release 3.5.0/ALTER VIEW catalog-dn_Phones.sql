/****** Object:  View [catalog].[dn_Phones]    Script Date: 07/23/2012 10:41:10 ******/
SET ANSI_NULLS ON
GO
...
SELECT     d .DeviceGuid, d .DeviceGuid AS ProductGuid, ISNULL(p.ProductId, 0) AS phoneID, ISNULL(p.ProductId, 0) AS product_id, ISNULL(p.ProductId, 0) AS ProductID, 
                      ...
                      ISNULL
                          ...
                          ((SELECT     LTRIM(RTRIM(Value)) AS Expr1
                              ...
                              , (SELECT TOP 1 p.Value FROM catalog.Property p WHERE Name = 'MetaKeywords' AND p.ProductGuid = d .DeviceGuid) MetaKeywords
                              , (SELECT TOP 1 p.Value FROM catalog.Property p WHERE Name = 'MetaDescription' AND p.ProductGuid = d .DeviceGuid) MetaDescription
