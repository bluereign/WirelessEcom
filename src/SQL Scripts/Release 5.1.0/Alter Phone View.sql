...
/****** Object:  View [catalog].[dn_Phones]    Script Date: 09/10/2013 17:19:57 ******/
SET ANSI_NULLS ON
GO
...
Title (DeviceGuid, pageTitle, summaryTitle, detailTitle, sd, ld, mk, md, bWarehouse, bfreebit) AS 
		...
		INNER JOIN catalog.property AS p4 WITH (NOLOCK) ON p4.ProductGuid = d.DeviceGuid AND p4.Name = 'MetaKeywords'
			AND p4.ProductGuid = p.ProductGuid 
			AND p4.ProductGuid = p2.ProductGuid AND p4.ProductGuid = p3.ProductGuid
		INNER JOIN catalog.property AS p5 WITH (NOLOCK) ON p5.ProductGuid = d.DeviceGuid 
