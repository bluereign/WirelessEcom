...
/****** Object:  View [catalog].[dn_PrePaids]    Script Date: 09/10/2013 17:20:17 ******/
SET ANSI_NULLS ON
GO
...
/* carriers only*/
ALTER VIEW [catalog].[dn_PrePaids] AS
SELECT     
	d.DeviceGuid
	...
	, CONVERT(bit, ISNULL((SELECT     1
		  ...
		  WHERE     (Tag = 'warehouse') AND (ProductGuid = d .DeviceGuid)), 0)) AS bWarehouse
	, CONVERT(bit, ISNULL
