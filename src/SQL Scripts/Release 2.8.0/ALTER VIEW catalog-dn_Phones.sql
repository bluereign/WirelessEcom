...
/****** Object:  View [catalog].[dn_Phones]    Script Date: 02/28/2012 17:06:38 ******/
SET ANSI_NULLS ON
GO
...
SELECT     d .DeviceGuid, d .DeviceGuid AS ProductGuid, ISNULL(p.ProductId, 0) AS phoneID, ISNULL(p.ProductId, 0) AS product_id, ISNULL(p.ProductId, 0) AS ProductID, 
                      ...
                      ISNULL
                          ...
                          ((SELECT     LTRIM(RTRIM(Value)) AS Expr1
                              ...
                              , ISNULL((SELECT LTRIM(RTRIM(Value)) FROM catalog.Property WHERE (Name = 'MetaKeywords') AND (Value <> '') AND (ProductGuid = d .DeviceGuid)), NULL) AS MetaKeywords
                              , ISNULL((SELECT LTRIM(RTRIM(Value)) FROM catalog.Property WHERE (Name = 'MetaDescription') AND (Value <> '') AND (ProductGuid = d .DeviceGuid)), NULL) AS MetaDescription
                              , 0 AS prepaid
