...
DELETE FROM catalog.ProductTag WHERE ProductGuid IN (SELECT ProductGuid FROM cjtmo.ProductTag);

MERGE catalog.ProductTag AS TARGET
USING (SELECT DISTINCT
			cpt.ProductGuid
			,Tag
		FROM cjtmo.ProductTag cpt
		INNER JOIN cjtmo.ProductGuid cpg ON cpg.ProductGuid = cpt.ProductGuid) AS SOURCE
...
update catalog.property set lastmodifiedby = 'CJAuto' where name = 'shortdescription' and lastmodifiedby = 'rlinmark'
and productguid in (select productguid from cjtmo.product)

UPDATE catalog.property
SET value = '<b>FREE</b> Accessories - <a title="Costco Online Value" rel="lightbox" href="/assets/common/images/onlinebenefit/costcoValue_Tmo.jpg">Click for Details</a><br>
<b>FREE</b> $25 Costco Cash Card*<br>
<b>FREE</b> Shipping<br>'
WHERE name = 'shortdescription' and lastmodifiedby = 'cjauto'
...
WHILE (@count <=(SELECT COUNT(*) FROM @table))
        ...
        EXEC msdb.dbo.sp_send_dbmail
			...
			@body= @Messaging,
			@body_format = 'HTML',
			@subject ='T-Mobile: CJ Catalog COSTCO Test DB Import Status',
