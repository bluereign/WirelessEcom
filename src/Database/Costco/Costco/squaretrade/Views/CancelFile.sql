...
CREATE VIEW [squaretrade].[CancelFile] AS 
...
SELECT DISTINCT
	...
	,CONVERT(VARCHAR(10),GETDATE()-1, 110) AS 'cancelDate'
...
WHERE
st.OrderId IN (SELECT OrderId FROM admin.OrderNote
WHERE
--CAST(DateCreated AS DATE) = CONVERT(VARCHAR(10),GETDATE()-1, 110) AND
OrderNoteSubjectId IN (
'2'
,'3'
,'8'
,'10'
...
,'22'
,'26'
)) AND st.OrderId NOT IN (SELECT Orderid FROM squaretrade.cancelfilelog
