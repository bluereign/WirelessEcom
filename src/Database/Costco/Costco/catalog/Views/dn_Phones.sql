...
*  Example Call:
              ...
              SELECT * FROM [catalog].[dn_Phones] WHERE ProductId = 4032
...
WITH onlineAvail( ProductGUID, Cnt )
  ...
  WHERE 
	(detail.TypeId = 1  -- Phones
		AND detail.Active = 1
		AND detail.Prepaid = 0
		AND detail.PageTitle IS NOT NULL
		AND detail.CarrierId <> '128')
	OR (detail.Buyurl IS NOT NULL AND detail.Active = '1' AND detail.CarrierId = '128')
;