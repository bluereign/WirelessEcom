...
LEFT JOIN catalog.Product cp ON cp.ProductGuid = i.ReferenceGuid AND cp.ChannelID = '2' AND i.IsActive = '1'
WHERE
(cp.ChannelId = '2' AND i.binImage IS NOT NULL AND i.IsActive = '1')
OR pp.ProductGuid IN (SELECT ProductGuid FROM catalog.Product WHERE ChannelID = '2')
OR pp.ParentProductGuid IN (SELECT ProductGuid FROM catalog.Product WHERE ChannelID = '2')
