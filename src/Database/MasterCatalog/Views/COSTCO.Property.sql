...
CREATE VIEW [COSTCO].[Property] AS
...
SELECT DISTINCT
	...
	,p.LastModifiedBy
	,p.Name
	,CASE WHEN p.Name = 'Inventory.HoldBackQty' THEN (SELECT DISTINCT Value FROM catalog.Property INNER JOIN ParentGuid ON ParentGuid.ProductGuid = catalog.Property.ProductGuid WHERE catalog.Property.ProductGuid = pp.ProductGuid AND catalog.property.Name = 'Inventory.HoldBackQty')
...
WHERE ProductGuid IN (SELECT DISTINCT ProductGuid FROM catalog.Product WHERE ChannelID = '2')
AND cpy.Name NOT IN (SELECT DISTINCT mp.Name FROM MasterProperties mp WHERE cpy.ProductGuid = mp.ProductGuid)
