...
CREATE VIEW [catalog].[dn_MasterTablets] AS

SELECT cd.name, c.CarrierId, cp.ChannelID, cd.TabletGuid, cpg.ProductTypeId, ISNULL(cpc.ParentProductGuid,cp.ProductGuid) AS 'MatchingGUID', cd.UPC, ISNULL(c.CompanyName, '') AS Carrier, ISNULL(cpy.Value, cd.Name) AS Title, ISNULL(cp.GersSku,'') AS 'GersSKU', cp.ProductId, cch.Channel, cp.Active, cp.CreateDate
                           FROM catalog.Product cp
                           INNER JOIN catalog.ProducttoParentChannel cpc ON cpc.ParentProductGuid = cp.ProductGuid
                           INNER JOIN catalog.Tablet cd ON cd.TabletGuid = cp.ProductGuid
                           ...
                           UNION 
                           SELECT  cd.name, c.CarrierId, cp.ChannelID, cd.TabletGuid, cpg.ProductTypeId, ISNULL(cpcc.ParentProductGuid,cp.ProductGuid) AS 'MatchingGUID', cd.UPC, ISNULL(c.CompanyName, '') AS Carrier
                           ,ISNULL(cpy.Value,(
                           ...
                           INNER JOIN catalog.Tablet d ON d.TabletGuid = p.ProductGuid AND d.Name IS NOT NULL
...
UNION
SELECT cd.name, c.CarrierId, cp.ChannelID, cd.TabletGuid, cpg.ProductTypeId, cp.ProductGuid AS 'MatchingGUID', cd.UPC, ISNULL(c.CompanyName, '') AS Carrier, ISNULL(cd.Name,'') AS Title, ISNULL(cp.GersSku,'') AS 'GersSKU', cp.ProductId, cch.Channel, cp.Active, cp.CreateDate
                           FROM catalog.Product cp
                           INNER JOIN catalog.Tablet cd ON cd.TabletGuid = cp.ProductGuid
