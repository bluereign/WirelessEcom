...
SET ANSI_NULLS ON
GO
CREATE VIEW [catalog].[dn_MasterPhones] AS

SELECT cd.name, c.CarrierId, cp.ChannelID, cd.deviceGuid, cpg.ProductTypeId, ISNULL(cpc.ParentProductGuid,cp.ProductGuid) AS 'MatchingGUID', cd.UPC, ISNULL(c.CompanyName, '') AS Carrier, ISNULL(cpy.Value, cd.Name) AS Title, ISNULL(cp.GersSku,'') AS 'GersSKU', cp.ProductId, cch.Channel, cp.Active, cp.CreateDate
                           ...
                           UNION 
                           SELECT  cd.name, c.CarrierId, cp.ChannelID, cd.deviceGuid, cpg.ProductTypeId, ISNULL(cpcc.ParentProductGuid,cp.ProductGuid) AS 'MatchingGUID', cd.UPC, ISNULL(c.CompanyName, '') AS Carrier
                           ,ISNULL(cpy.Value,(
                           ...
                           INNER JOIN catalog.Device d ON d.DeviceGuid = p.ProductGuid AND d.Name IS NOT NULL
...
UNION
SELECT cd.name, c.CarrierId, cp.ChannelID, cd.deviceGuid, cpg.ProductTypeId, cp.ProductGuid AS 'MatchingGUID', cd.UPC, ISNULL(c.CompanyName, '') AS Carrier, ISNULL(cd.Name,'') AS Title, ISNULL(cp.GersSku,'') AS 'GersSKU', cp.ProductId, cch.Channel, cp.Active, cp.CreateDate
                           FROM catalog.Product cp
                           INNER JOIN catalog.Device cd ON cd.DeviceGuid = cp.ProductGuid
