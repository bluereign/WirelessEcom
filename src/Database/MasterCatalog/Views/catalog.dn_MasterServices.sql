...
CREATE VIEW [catalog].[dn_MasterServices] AS

SELECT cd.Title AS 'Name', c.CarrierId, cp.ChannelID, cd.ServiceGuid, cpg.ProductTypeId, ISNULL(cpc.ParentProductGuid,cp.ProductGuid) AS 'MatchingGUID', cd.CarrierBillCode AS 'SOC Code', ISNULL(c.CompanyName, '') AS Carrier, ISNULL(cpy.Value, cd.Title) AS Title, ISNULL(cp.GersSku,'') AS 'GersSKU', cp.ProductId, cch.Channel, cp.Active, cp.CreateDate
                           FROM catalog.Product cp
                           INNER JOIN catalog.ProducttoParentChannel cpc ON cpc.ParentProductGuid = cp.ProductGuid
                           INNER JOIN catalog.Service cd ON cd.ServiceGuid = cp.ProductGuid
                           ...
                           UNION 
                           SELECT  cd.Title, c.CarrierId, cp.ChannelID, cd.ServiceGuid, cpg.ProductTypeId, ISNULL(cpcc.ParentProductGuid,cp.ProductGuid) AS 'MatchingGUID', cd.CarrierBillCode AS 'SOC COde', ISNULL(c.CompanyName, '') AS Carrier
                           ,ISNULL(cpy.Value,(
                           ...
                           INNER JOIN catalog.Service d ON d.ServiceGuid = p.ProductGuid AND d.Title IS NOT NULL
...
UNION
SELECT cd.Title AS 'Name', c.CarrierId, cp.ChannelID, cd.ServiceGuid, cpg.ProductTypeId, cp.ProductGuid AS 'MatchingGUID', cd.CarrierBillCode AS 'SOC Code', ISNULL(c.CompanyName, '') AS Carrier, ISNULL(cd.Title,'') AS Title, ISNULL(cp.GersSku,'') AS 'GersSKU', cp.ProductId, cch.Channel, cp.Active, cp.CreateDate
                           FROM catalog.Product cp
                           INNER JOIN catalog.Service cd ON cd.ServiceGuid = cp.ProductGuid
