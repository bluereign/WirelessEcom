...
---------------------------------------------------------------------------- 
-- Object Type : Trigger
-- Object Name : msdb.tr_lograteplanmarket
-- Description : Monitor all changes in the catalog.rateplanmarket table
-- Author : Naomi Hall
-- Date : December 18th, 2012 -- Edited January 3, 2013
---------------------------------------------------------------------------- 

--The Trigger
CREATE TRIGGER [catalog].[tr_lograteplanmarket] ON [catalog].[RateplanMarket] FOR INSERT, UPDATE, DELETE
...
SELECT @TableName = 'rateplanmarket'
...
SELECT @PKValue = a FROM @t2
	...
	END
GO
ALTER TABLE [catalog].[RateplanMarket] ADD CONSTRAINT [PK_RateplanMarket] PRIMARY KEY CLUSTERED  ([RateplanGuid], [MarketGuid]) WITH (FILLFACTOR=80) ON [PRIMARY]
GO
ALTER TABLE [catalog].[RateplanMarket] ADD CONSTRAINT [FK_RateplanMarket_Market] FOREIGN KEY ([MarketGuid]) REFERENCES [catalog].[Market] ([MarketGuid])
GO
ALTER TABLE [catalog].[RateplanMarket] WITH NOCHECK ADD CONSTRAINT [FK_RateplanMarket_Rateplan] FOREIGN KEY ([RateplanGuid]) REFERENCES [catalog].[Rateplan] ([RateplanGuid])
GO
EXEC sp_addextendedproperty N'MS_Description', N'Link to Market table', 'SCHEMA', N'catalog', 'TABLE', N'RateplanMarket', 'COLUMN', N'MarketGuid'
GO
EXEC sp_addextendedproperty N'MS_Description', N'Link to Rateplan table', 'SCHEMA', N'catalog', 'TABLE', N'RateplanMarket', 'COLUMN', N'RateplanGuid'
GO
