...
SELECT @PKValue = a FROM @t2
	...
	END
GO
ALTER TABLE [catalog].[Service] ADD CONSTRAINT [PK_Service] PRIMARY KEY CLUSTERED  ([ServiceGuid]) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [IX_Service] ON [catalog].[Service] ([ServiceGuid]) WITH (FILLFACTOR=80) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_ServiceGuid_CarrierBillCode_MonthlyFee] ON [catalog].[Service] ([ServiceGuid], [CarrierBillCode], [MonthlyFee]) WITH (FILLFACTOR=80) ON [PRIMARY]
GO
ALTER TABLE [catalog].[Service] ADD CONSTRAINT [FK_Service_Company] FOREIGN KEY ([CarrierGuid]) REFERENCES [catalog].[Company] ([CompanyGuid])
GO
ALTER TABLE [catalog].[Service] WITH NOCHECK ADD CONSTRAINT [FK_Service_ProductGuidType] FOREIGN KEY ([ServiceGuid]) REFERENCES [catalog].[ProductGuid] ([ProductGuid])
GO
EXEC sp_addextendedproperty N'MS_Description', N'The code to be sent to the carrier when activating the service. This can be null for included services.', 'SCHEMA', N'catalog', 'TABLE', N'Service', 'COLUMN', N'CarrierBillCode'
GO
EXEC sp_addextendedproperty N'MS_Description', N'Link to Company table', 'SCHEMA', N'catalog', 'TABLE', N'Service', 'COLUMN', N'CarrierGuid'
GO
EXEC sp_addextendedproperty N'MS_Description', N'Used by the catalog loader to match services in the incoming catalog to existing services', 'SCHEMA', N'catalog', 'TABLE', N'Service', 'COLUMN', N'CarrierServiceId'
GO
EXEC sp_addextendedproperty N'MS_Description', N'Link to ProductGuid table', 'SCHEMA', N'catalog', 'TABLE', N'Service', 'COLUMN', N'ServiceGuid'
GO
