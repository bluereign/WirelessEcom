...
(
[PropertyMasterGroupGuid] [uniqueidentifier] NOT NULL ROWGUIDCOL CONSTRAINT [DF_Table_1_PropertyMasterGroupId] DEFAULT (newid()),
...
---------------------------------------------------------------------------- 
-- Object Type : Trigger
-- Object Name : msdb.tr_logpropertymastergroup
-- Description : Monitor all changes in the catalog.propertymastergroup table
-- Author : Naomi Hall
-- Date : December 18th, 2012 -- Edited January 3, 2013
---------------------------------------------------------------------------- 

--The Trigger
CREATE TRIGGER [catalog].[tr_logpropertymastergroup] ON [catalog].[propertymastergroup] FOR INSERT, UPDATE, DELETE
...
SELECT @TableName = 'propertymastergroup'
...
SELECT @PKValue = a FROM @t
	...
	END
GO
ALTER TABLE [catalog].[PropertyMasterGroup] ADD CONSTRAINT [PK_PropertyMasterGroup] PRIMARY KEY CLUSTERED  ([PropertyMasterGroupGuid]) WITH (FILLFACTOR=80) ON [PRIMARY]
GO
EXEC sp_addextendedproperty N'MS_Description', N'Primary key', 'SCHEMA', N'catalog', 'TABLE', N'PropertyMasterGroup', 'COLUMN', N'PropertyMasterGroupGuid'
GO
