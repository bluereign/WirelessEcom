...
(
[ServiceMasterGuid] [uniqueidentifier] NOT NULL ROWGUIDCOL CONSTRAINT [DF_ServiceMaster_ServiceMasterId] DEFAULT (newid()),
...
---------------------------------------------------------------------------- 
-- Object Type : Trigger
-- Object Name : msdb.tr_logservicemaster
-- Description : Monitor all changes in the catalog.servicemaster table
-- Author : Naomi Hall
-- Date : December 18th, 2012 -- Edited January 3, 2013
---------------------------------------------------------------------------- 

--The Trigger
CREATE TRIGGER [catalog].[tr_logservicemaster] ON [catalog].[ServiceMaster] FOR INSERT, UPDATE, DELETE
...
SELECT @TableName = 'servicemaster'
...
SELECT @PKValue = a FROM @t2
	...
	END
GO
ALTER TABLE [catalog].[ServiceMaster] ADD CONSTRAINT [PK_ServiceMaster] PRIMARY KEY CLUSTERED  ([ServiceMasterGuid]) WITH (FILLFACTOR=80) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_ServiceManagerGroupLookups] ON [catalog].[ServiceMaster] ([ServiceMasterGroupGuid], [ServiceGUID], [Label], [ServiceMasterGuid], [Ordinal]) WITH (FILLFACTOR=80) ON [PRIMARY]
GO
ALTER TABLE [catalog].[ServiceMaster] WITH NOCHECK ADD CONSTRAINT [FK_ServiceMaster_ServiceMasterGroup] FOREIGN KEY ([ServiceMasterGroupGuid]) REFERENCES [catalog].[ServiceMasterGroup] ([ServiceMasterGroupGuid])
GO
EXEC sp_addextendedproperty N'MS_Description', N'Link to ServiceMasterGroup table', 'SCHEMA', N'catalog', 'TABLE', N'ServiceMaster', 'COLUMN', N'ServiceMasterGroupGuid'
GO
EXEC sp_addextendedproperty N'MS_Description', N'Primary key', 'SCHEMA', N'catalog', 'TABLE', N'ServiceMaster', 'COLUMN', N'ServiceMasterGuid'
GO
