...
(
[ServiceMasterGroupGuid] [uniqueidentifier] NOT NULL ROWGUIDCOL CONSTRAINT [DF_Table_1_ServiceMasterGroupId] DEFAULT (newid()),
...
[Ordinal] [int] NULL,
[IsShared] [bit] NULL CONSTRAINT [DF__ServiceMa__IsSha__09EF578A] DEFAULT (NULL)
) ON [PRIMARY]
GO
...
---------------------------------------------------------------------------- 
-- Object Type : Trigger
-- Object Name : msdb.tr_logservicemastergroup
-- Description : Monitor all changes in the catalog.servicemastergroup table
-- Author : Naomi Hall
-- Date : December 18th, 2012 -- Edited January 3, 2013
---------------------------------------------------------------------------- 

--The Trigger
CREATE TRIGGER [catalog].[tr_logservicemastergroup] ON [catalog].[ServiceMasterGroup] FOR INSERT, UPDATE, DELETE
...
SELECT @TableName = 'servicemastergroup'
...
SELECT @PKValue = a FROM @t2
	...
	END
GO
ALTER TABLE [catalog].[ServiceMasterGroup] ADD CONSTRAINT [PK_ServiceMasterGroup] PRIMARY KEY CLUSTERED  ([ServiceMasterGroupGuid]) WITH (FILLFACTOR=80) ON [PRIMARY]
GO
EXEC sp_addextendedproperty N'MS_Description', N'Primary key', 'SCHEMA', N'catalog', 'TABLE', N'ServiceMasterGroup', 'COLUMN', N'ServiceMasterGroupGuid'
GO
EXEC sp_addextendedproperty N'MS_Description', N'I=Included, R=Required, O=Optional', 'SCHEMA', N'catalog', 'TABLE', N'ServiceMasterGroup', 'COLUMN', N'Type'
GO
