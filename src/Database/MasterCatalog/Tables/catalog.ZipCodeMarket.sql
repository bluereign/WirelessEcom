...
---------------------------------------------------------------------------- 
-- Object Type : Trigger
-- Object Name : msdb.tr_logzipcodemarket
-- Description : Monitor all changes in the catalog.zipcodemarket table
-- Author : Naomi Hall
-- Date : December 18th, 2012 -- Edited January 3, 2013
---------------------------------------------------------------------------- 

--The Trigger
CREATE TRIGGER [catalog].[tr_logzipcodemarket] ON [catalog].[ZipCodeMarket] FOR INSERT, UPDATE, DELETE
...
SELECT @TableName = 'zipcodemarket'
...
SELECT @PKValue = a FROM @t2
	...
	END
GO
ALTER TABLE [catalog].[ZipCodeMarket] ADD CONSTRAINT [PK_ZipCodeMarket] PRIMARY KEY CLUSTERED  ([ZipCode], [MarketGuid]) WITH (FILLFACTOR=80) ON [PRIMARY]
GO
ALTER TABLE [catalog].[ZipCodeMarket] ADD CONSTRAINT [FK_ZipCodeMarket_Market] FOREIGN KEY ([MarketGuid]) REFERENCES [catalog].[Market] ([MarketGuid])
GO
EXEC sp_addextendedproperty N'MS_Description', N'Link to Market table', 'SCHEMA', N'catalog', 'TABLE', N'ZipCodeMarket', 'COLUMN', N'MarketGuid'
GO
EXEC sp_addextendedproperty N'MS_Description', N'Primary key', 'SCHEMA', N'catalog', 'TABLE', N'ZipCodeMarket', 'COLUMN', N'ZipCode'
GO
