...
ALTER TABLE [catalog].[RateplanService] WITH NOCHECK ADD
...
---------------------------------------------------------------------------- 
-- Object Type : Trigger
-- Object Name : msdb.tr_lograteplanservice
-- Description : Monitor all changes in the catalog.rateplanservice table
-- Author : Naomi Hall
-- Date : December 18th, 2012 -- Edited January 3, 2013
---------------------------------------------------------------------------- 

--The Trigger
CREATE TRIGGER [catalog].[tr_lograteplanservice] ON [catalog].[RateplanService] FOR INSERT, UPDATE, DELETE
...
SELECT @TableName = 'rateplanservice'
...
SELECT @PKValue = a FROM @t2
	...
	END
GO
ALTER TABLE [catalog].[RateplanService] ADD CONSTRAINT [PK_RateplanService] PRIMARY KEY CLUSTERED  ([RateplanGuid], [ServiceGuid]) WITH (FILLFACTOR=80) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_ServiceGuid_RateplanGuid_IsIncluded] ON [catalog].[RateplanService] ([ServiceGuid], [RateplanGuid], [IsIncluded]) WITH (FILLFACTOR=80) ON [PRIMARY]
GO
ALTER TABLE [catalog].[RateplanService] WITH NOCHECK ADD CONSTRAINT [FK_RateplanService_Rateplan] FOREIGN KEY ([RateplanGuid]) REFERENCES [catalog].[Rateplan] ([RateplanGuid])
GO

EXEC sp_addextendedproperty N'MS_Description', N'Link to Rateplan table', 'SCHEMA', N'catalog', 'TABLE', N'RateplanService', 'COLUMN', N'RateplanGuid'
GO
EXEC sp_addextendedproperty N'MS_Description', N'Link to Service table', 'SCHEMA', N'catalog', 'TABLE', N'RateplanService', 'COLUMN', N'ServiceGuid'
GO
