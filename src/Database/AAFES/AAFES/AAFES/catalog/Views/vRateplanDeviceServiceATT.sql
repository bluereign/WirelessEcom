...
CREATE VIEW [catalog].[vRateplanDeviceServiceATT]
AS
-- AT&T Device Services
...
WHERE     (C.CompanyName = N'AT&T') AND (RS.ServiceGuid IS NULL)

UNION ALL

-- AT&T RateplanDevice Services
SELECT     R.RateplanGuid, D.DeviceGuid, S.ServiceGuid, R.CarrierBillCode AS RateplanBillCode, D.UPC, S.CarrierGuid, C.CompanyName AS CarrierName, 
...
WHERE     (C.CompanyName = N'AT&T') AND (RS.IsIncluded = 0)

UNION ALL

-- AT&T Rateplan Services (Included)
SELECT     R.RateplanGuid, D.DeviceGuid, S.ServiceGuid, R.CarrierBillCode AS RateplanBillCode, D.UPC, S.CarrierGuid, C.CompanyName AS CarrierName, 
                      S.CarrierBillCode AS ServiceBillCode, S.Title, S.MonthlyFee, RS.IsIncluded
...
WHERE     (C.CompanyName = N'AT&T') AND (RS.IsIncluded = 1)