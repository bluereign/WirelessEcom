...
INNER JOIN SquareTrade st ON st.OrderId = so.OrderId AND
st.GroupNumber IN (SELECT GroupNumber FROM squaretrade WHERE GersSku = 'cwshppec' AND OrderId = so.OrderId)
INNER JOIN catalog.Company cc1 ON cc1.CarrierId = so.CarrierId
INNER JOIN catalog.Product cp ON cp.ProductId = st.ProductId
...
so.Status = '3'
AND so.OrderId NOT IN (SELECT Orderid FROM squaretrade.addfilelog
)
