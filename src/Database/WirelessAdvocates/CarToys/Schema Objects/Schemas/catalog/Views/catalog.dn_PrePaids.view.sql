...
/* carriers only*/
CREATE VIEW [catalog].[dn_PrePaids]
AS
SELECT     d .DeviceGuid, d .DeviceGuid AS ProductGuid, ISNULL(p.ProductId, 0) AS phoneID, ISNULL(p.ProductId, 0) AS product_id, ISNULL(p.ProductId, 0) 
                      ...
                      0 AS lrgimageheight, ISNULL
                          ...
                          ((SELECT     ltrim(rtrim(Value))
                              FROM         catalog.Property
                              WHERE     (Name = 'LongDescription') AND (Value <> '') AND (ProductGuid = d .DeviceGuid)), NULL) AS detailDescription, NULL 
                      AS metaKeywords, NULL AS metaDescription, 1 AS prepaid, pt.ProductTypeId AS typeID, ISNULL
                          ((SELECT     gp.Price
...
FROM         catalog.Device AS d INNER JOIN
                      ...
                      catalog.ProductTag ptag ON p.ProductGuid = ptag.ProductGuid AND ptag.Tag = 'prepaid'
WHERE     p.Active = 1
