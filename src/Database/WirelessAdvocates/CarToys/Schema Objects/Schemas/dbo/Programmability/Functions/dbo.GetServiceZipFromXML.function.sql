...
CREATE FUNCTION [dbo].[GetServiceZipFromXML] (@referenceNumber varchar(30))  
...
BEGIN 
	...
	FROM	service.CarrierInterfaceLog
	WHERE	ReferenceNumber = @referenceNumber
	AND		RequestType = 'CreditCheck'
	AND		Carrier = 'Verizon'
	AND		[Type] = 'Request'
	ORDER BY id DESC
	
	-- query for the mdn
	SELECT @mdn = CAST(@requestXml.query('declare namespace OAS="http://www.verizonwireless.com/oas";
	data(/OAS:retailOrder/OAS:order/OAS:subOrder/OAS:retailCreditRequest/OAS:systemData/OAS:serviceZipCode)') AS VARCHAR(30))
END
return @mdn
END*/
