<cfcomponent output="false" displayname="CartHelper">

	<cffunction name="init" returntype="CartHelper">
		<cfparam name="session.cart" type="any" default="#createObject('component','cfc.model.Cart').init()#">
	...
	<cffunction name="addLineToCart" returntype="numeric">
		...
		<cfset arrayAppend(session.cart.getLines(),createObject("component","CartLine").init())>
	...
	<cffunction name="addLine" returntype="void">
		...
		<cfif arrayLen(session.cart.getLines()) lt request.config.maxLines>
			<cfset arrayAppend(session.cart.getLines(),createObject("component","cfc.model.CartLine").init())>
			<cfset session.cart.setCurrentLine(arrayLen(session.cart.getLines()))>
			<cfif session.cart.getFamilyPlan().hasBeenSelected()>
	...
	<cffunction name="removeFamilyPlan" returntype="void">
		...
		<cfloop from="1" to="#arrayLen(local.cartLines)#" index="local.iLine">
			<cfset local.thisLine = local.cartLines[local.iLine]>
			<cfset local.thisLine.setPlan(createObject("component","cfc.model.CartItem").init())>
		</cfloop>
	...
	<cffunction name="removePhone" access="public" output="false" returntype="void">
		...
		<cfif arguments.line lte arrayLen(local.cartLines)>
			<cfset local.cartLine = local.cartLines[arguments.line] />
			<cfset local.cartLine.setPhone(createObject('component', 'cfc.model.CartItem').init()) />
	...
	<cffunction name="removePlan" access="public" output="false" returntype="void">
		...
		<cfelse>
			...
			<cfif arguments.line lte arrayLen(local.cartLines)>
				<cfset local.cartLine = local.cartLines[arguments.line] />
				<cfset local.cartLine.setPlan(createObject('component', 'cfc.model.CartItem').init()) />
				<cfset local.cartLine.setFeatures(arrayNew(1)) />
			</cfif>
		</cfif>
	...
	<cffunction name="removeWarranty" access="public" output="false" returntype="void">
		...
		<cfif arguments.line lte arrayLen(local.cartLines)>
			<cfset local.cartLine = local.cartLines[arguments.line] />
			<cfset local.cartLine.setWarranty(createObject('component', 'cfc.model.CartItem').init()) />
			<cfset request.hasWirelessItemBeenAdded = true />
	...
	<cffunction name="validateCartForCheckout" access="public" output="false" returntype="cfc.model.CartValidationResponse">
		<cfset var local = structNew()>
		<cfset local.cartValidationResponse = createObject("component","cfc.model.CartValidationResponse").init()>
	...
	<cffunction name="addCartLineDevice" access="public" output="false" returntype="void">
		...
		<cfscript>
			...
			if (local.freeAccessories.recordCount)
			{
				...
				{
					local.thisFreeAccessory = createObject("component","cfc.model.CartItem").init();
...
//			session.prePaidFilterSelections.carrierID = local.thisPlan.carrierID;
			...
			if (len(trim(arguments.featureIds)) and local.cartLines[arguments.line].getPhone().hasBeenSelected())
			{
				...
				for (local.i=1;local.i lte listLen(arguments.featureIds);local.i=local.i+1)
				{
					...
					{
						arrayAppend(local.arrFeatures,createObject("component","cfc.model.CartItem").init());
	...
	<cffunction name="addCartLineAccessory" access="public" output="false" returntype="void">
		...
		<cfscript>
			...
			local.newAccessory = createObject("component","cfc.model.CartItem").init();
		...
		<cfscript>
			...
			if ( qPlan.RecordCount )
			{
				...
				if ( qDefaultServices.RecordCount )
				{
					...
					{
						featureItem = CreateObject( 'component', 'cfc.model.CartItem' ).init();
