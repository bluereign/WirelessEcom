<cfcomponent name="Coupon" output="false">
	...
	<cffunction name="init" access="public" returntype="Coupon" output="false">
		...
		<cfset this.model = createObject('component', 'cfc.model.coupon').init() />
			...
			<script type="text/javascript">
				...
				function validateAdd(theForm)	{
					...
					if(!theForm.couponCode.value.length)	{
						...
						return false;
					} else if(!theForm.endDate.value.length || !validateUSDate(theForm.endDate.value))	{
						alert('Please enter a valid end date.');
						theForm.endDate.focus();

						return false;
					} else if(new Date(theForm.startDate.value) >= new Date(theForm.endDate.value))	{
				...
				<table width="100%" cellpadding="3" cellspacing="0" border="0">
					<form action="index.cfm?c=#url.c#" method="post" name="frmAddCoupon" onsubmit="return validateAdd(this.name)">
						...
						<tr valign="top">
							...
							<td style="padding: 3px"><input type="text" name="discountValue" style="width: 50px" /></td>
						</tr>
						<tr>
				...
				<table width="100%" cellpadding="3" cellspacing="0" border="0">
					<form action="index.cfm?c=#url.c#" method="post" name="frmEditCoupon" onsubmit="return validateEdit(this.name)">
						...
						<tr valign="top">
							<td width="100" style="padding: 3px">End Date/Time:</td>
							<td style="padding: 3px">
								<input type="text" id="endDatePicker" name="endDate" value="<cfif isDate(qry_getCoupon.validEndDate)>#dateFormat(qry_getCoupon.validEndDate, 'mm/dd/yyyy')#</cfif>" style="width: 80px" />
								<select name="endTime">
									<cfloop from="0" to="23" index="idx">
						...
						<tr valign="top">
							<td width="100" style="padding: 3px">Coupon Value:</td>
							<td style="padding: 3px"><input type="text" name="discountValue" value="#val(qry_getCoupon.discountValue)#" style="width: 50px" /></td>
						</tr>
						<tr>
							<td colspan="2" style="padding: 3px">&nbsp;</td>
						</tr>
	...
	<cffunction name="isCouponValid" access="public" returntype="string" output="false">
		...
		<cfreturn this.model.isCouponValid(argumentCollection = arguments) />
	</cffunction>

	<cffunction name="checkCouponCode" access="public" returntype="string" output="false">
		<cfargument name="couponCode" required="true" type="string" />

		<cfset var error = '' />
		<cfset var checkCouponCodeReturn = '' />
		<cfset var qry_getCoupon = queryNew('undefined') />

		<cfif not len(trim(arguments.couponCode))>
			<cfset error = 'false|The coupon code you provided is not valid.' />
		<cfelseif listFirst(isCouponValid(trim(arguments.couponCode)), '|') is 'FALSE'>
			<cfset error = 'false|The coupon code you provided is not valid.' />
		</cfif>

		<cfif len(trim(error))>
			<cfset checkCouponCodeReturn = error />
		<cfelse>
			<cfset application.model.checkOutHelper.setCheckoutCouponCode(trim(arguments.couponCode)) />
			<cfset qry_getCoupon = getCoupon(couponCode = trim(arguments.couponCode)) />
			<cfset checkCouponCodeReturn = 'true|You have entered the following code.' />
		</cfif>

		<cfreturn checkCouponCodeReturn />
	</cffunction>
</cfcomponent>