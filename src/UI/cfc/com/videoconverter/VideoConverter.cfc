...
<cffunction name="convertVideo" access="public" returntype="string" output="no" hint="I convert a Video to the requested format. I return the file name">
	...
	<cfset outputFilePath = Arguments.outputFilePath />
	
	<!--- Create directory if it does not exist --->
	<cfset Variables.FileMgr.makedir(GetDirectoryFromPath(outputFilePath)) />
...
<cffunction name="generateVideoThumb" access="public" returntype="string" output="no" hint="I generate a Video thumbnail JPEG.">
	...
	<cfargument name="Offset" type="string" default="1" required="false" />
	<cfargument name="ThumbHeight" type="numeric" default="0" required="false" />
	<cfargument name="ThumbWidth" type="numeric" default="0" required="false" />
	<cfargument name="OverwriteFile" type="boolean" default="true" required="false" />
	...
	<cfset var result = "">
	<cfset var ThumbFileName = getFileFromPath(ListDeleteAt(Arguments.VideoFilePath,ListLen(Arguments.VideoFilePath,"."),".") & ".png")>
	<cfset var imageDimension = "" />
	
	<!--- Determine the image path --->
	<cfif arguments.OverwriteFile>
		<cfset Arguments.ThumbFilePath = Variables.FileMgr.getFilePath(ThumbFileName, Arguments.ThumbFolder) />
	<cfelse>
	...
	<!--- Create directory if it does not exist --->
	<cfset Variables.FileMgr.makedir(GetDirectoryFromPath(Arguments.ThumbFilePath)) />

	<!--- Determine if image dimensions are valid --->
	<cfif Arguments.ThumbHeight && Arguments.ThumbWidth>
		<cfset imageDimension = "-s " & Arguments.ThumbHeight & "x" & Arguments.ThumbWidth />
	</cfif>
...
/*	try {*/
		...
		//command = '#Variables.LibraryPath#ffmpeg.exe -i -itsoffset -4 "#Arguments.VideoFilePath#" -vcodec mjpeg -vframes 1 -an -f rawvideo -s "#Arguments.ThumbFilePath#"';
		
		command = '#Variables.LibraryPath#ffmpeg.exe -itsoffset -#Arguments.Offset# -i "#Arguments.VideoFilePath#" -vcodec png -vframes 1 -an -f rawvideo #imageDimension# -y "#Arguments.ThumbFilePath#"';
