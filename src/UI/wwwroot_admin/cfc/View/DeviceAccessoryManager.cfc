<cfcomponent output="false" displayname="DeviceAccessoryManager">
	...
	<cffunction name="getEditAccessoryForDeviceForm" returntype="string" access="public">
		...
		<cfset var local = {
				...
				filter = {},
				accessoriesQry = application.model.AdminAccessory.getAccessoriesDropDown(local.filter)
			} />
	...
	<cffunction name="getAccessoryForDeviceDisplayList" access="public" returntype="string">
		...
		<cfset local.accessoryList = application.model.Catalog.getDeviceAccessories(local.productId) />

		<cfsavecontent variable="local.html">
			<script type="text/javascript">
				$(function() {
					$("#sortableAccessories").sortable({
						cursor: 'crosshair',
			...
			<a href="javascript: show('action=showEditAccessoryForm');" href="#" class="button"><span>Add New Accessory</span></a>
			<cfif local.accessoryList.RecordCount gt 0>
         	<form method="post" name="bulkAccessoryUpdate" class="middle-forms">
				<div id="accessoryListHeader">
	                <span id="accessoryName">Accessory Name</span>
	                <div style="clear:both;"></div>
				</div>
			   	<div id="accessoryList">
					<ul id="sortableAccessories">
...
--->
			    ...
			    <input type="hidden" value="<cfoutput>#local.productId#</cfoutput>" name="productId" />
			    <input type="hidden" value="bulkAccessoryUpdate" name="action">
			    <a href="javascript: void();" onclick="postForm(this);" class="button"><span>Save</span></a> <a href="javascript: show('action=cancelAccessoryForDeviceEdit');" class="button"><span>Cancel</span></a>
			</form>
				<p>
