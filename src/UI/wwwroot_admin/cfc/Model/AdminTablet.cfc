<cfcomponent output="false" displayname="AdminTablet">
		...
		<cftry>
			<cfquery name="local.isUPC" datasource="#application.dsn.wirelessadvocates#">
				...
				WHERE UPC = <cfqueryparam cfsqltype="cf_sql_varchar" value="#local.upc#" />
					...
					AND TabletGuid <> <cfqueryparam cfsqltype="cf_sql_varchar" value="#local.productGuid#" />
			</cfquery>
			<cfcatch type="any">
	...
	<cffunction name="cloneTablet" returntype="any">
		...
		<cfstoredproc procedure="clone.usp_Tablet" datasource="#application.dsn.wirelessadvocates#">
	...
	<cffunction name="cloneMasterTablet" returntype="any">
		<cfargument name="deviceGuid" type="string" />
		...
		<cfstoredproc procedure="clone.usp_MasterTablet" datasource="#application.dsn.wirelessadvocates#">
			<cfprocparam cfsqltype="CF_SQL_INTEGER" value="#arguments.productId#"><!--- ProductId --->
	...
	<cffunction name="getTablet" returntype="query">
		...
		<cftry>
			<cfquery name="local.getTablet" datasource="#application.dsn.wirelessadvocates#">
				select
					d.TabletGuid
					, d.Name
					, d.UPC
					...
					, p.ProductId
					, IsNull((select value from catalog.Property where Name = 'Title' and ProductGuid = d.TabletGuid),'') Title
					, IsNull((select value from catalog.Property where Name = 'ShortDescription' and ProductGuid = d.TabletGuid),'') ShortDescription
					, IsNull((select value from catalog.Property where Name = 'LongDescription' and ProductGuid = d.TabletGuid),'') LongDescription
					, IsNull((select value from catalog.Property where Name = 'MetaDescription' and ProductGuid = d.TabletGuid),'') MetaDescription
					, IsNull((select value from catalog.Property where Name = 'MetaKeywords' and ProductGuid = d.TabletGuid),'') MetaKeywords
					, IsNull((select value from catalog.Property where Name = 'ReleaseDate' and ProductGuid = d.TabletGuid),'') ReleaseDate
					, IsNull((select value from catalog.Property where Name = 'Inventory.HoldBackQty' and ProductGuid = d.TabletGuid),'') SafteyStock
					, p.channelID
					, ch.channel
				from catalog.tablet d
					inner join catalog.ProductGuid pg on pg.ProductGuid = d.TabletGuid
				...
				where
					d.TabletGuid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#local.deviceId#">
			</cfquery>
			<cfcatch type="any">
				<cfthrow message="#cfcatch.message# - #cfcatch.detail#" />
			</cfcatch>
		...
		<cfreturn local.getTablet>

	</cffunction>

	<!--- 
		Create a list of tablets that is sorted by matching guid so all tablets descended from the same master record are sorted together. 
		To facilitate this and to eliminate the need to code a monstrous union a new view (catalog.dn_MasterTablets) was created to simply
		the sql and maintain any time this union/join needs to be performed.
	--->
	<cffunction name="getTablets" access="public" returntype="query" output="false">
		...
		<cftry>
			...
			<cfif structKeyExists(local.filter, 'channel') and Len(local.filter.channel) and local.filter.channel gt 1 >	
				<cfquery name="local.getMatchingGuids" datasource="#application.dsn.wirelessadvocates#" >
					SELECT * FROM catalog.dn_MasterTablets                           
			...
			<cfquery name="local.getTablets" datasource="#application.dsn.wirelessadvocates#">
				SELECT * FROM catalog.dn_MasterTablets                           
				WHERE ProductTypeId =	6
				<cfif StructKeyExists(local.filter, "active") and Len(local.filter.active)>
					AND	ISNULL(Active, 0)	= <cfqueryparam value="#local.filter.active#" cfsqltype="cf_sql_integer" />
			...
			<!---
			 ...
			 --->
			<cfif structKeyExists(local,"getMatchingGuids") and local.getMatchingGuids.recordcount>
			<cfset qry = local.gettablets /> 
			<cfquery name="local.getTablets2" dbtype="query">
				select * from qry
				where MatchingGUID				
		...
		<cfif structKeyExists(local,"getTablets2")>
			<cfreturn local.getTablets2 />
		<cfelse>
			<cfreturn local.getTablets />
		</cfif>
	</cffunction>

	<cffunction name="insertTablet" returntype="string">
		...
		<cftry>
			<cfquery name="local.insertTablet" datasource="#application.dsn.wirelessadvocates#">
				INSERT INTO catalog.Tablet (
					TabletGuid,
					CarrierGuid,
					ManufacturerGuid,
					UPC,
	...
	<cffunction name="updateTablet" returntype="string">
		<cfargument name="form" type="struct" required="true" />
		...
		<cftry>
			<!--- updates table --->
			<cfquery name="local.updateTablet" datasource="#application.dsn.wirelessadvocates#">
				UPDATE catalog.Tablet
				...
				WHERE TabletGuid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#local.productGuid#" />
			</cfquery>
			<cfcatch type="any">
				<cfthrow message="#cfcatch.message# - #cfcatch.detail#" />
			</cfcatch>
	...
	<cffunction name="updateDefaultDevicePlan" output="false" access="public" returntype="string">
		...
		<cfquery datasource="#application.dsn.wirelessadvocates#">
			...
			WHERE DeviceGuid = <cfqueryparam value="#arguments.productGuid#" cfsqltype="cf_sql_varchar" />
			...
			WHERE
				...
				AND DeviceGuid = <cfqueryparam value="#arguments.productGuid#" cfsqltype="cf_sql_varchar" />
		</cfquery>

	</cffunction>

	<cffunction name="getMasterTablet" access="public" output="false" returntype="struct" hint="Returns the data for the Master Channel device pased on productID">
		...
		<cftry>
			<cfquery name="masterData" datasource="#application.dsn.wirelessadvocates#" >
				SELECT IsNull((select value from catalog.Property where Name = 'ShortDescription' and ProductGuid = d.TabletGuid),'') ShortDescription
					, IsNull((select value from catalog.Property where Name = 'LongDescription' and ProductGuid = d.TabletGuid),'') LongDescription
				FROM catalog.Tablet d
					INNER JOIN catalog.ProductGuid pg on pg.ProductGuid = d.TabletGuid
					LEFT JOIN catalog.Product p on p.ProductGuid = pg.ProductGuid
