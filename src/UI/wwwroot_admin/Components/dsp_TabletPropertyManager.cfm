...
<cfif len(productId) gt 0>
    ...
    <cfif IsDefined("form.action")>
        ...
        <cfelseif form.action eq "bulkUpdateAssigned">
			...
			<cfset result = application.model.PropertyManager.bulkUpdateValue(form)>
    ...
    <cfif showList eq true>
        <!--- get the properties --->
        <cfset featureProperties = application.model.PropertyManager.getPropertiesByProductId(url.productguid,"features",false,true)>
        ...
        <div>
			...
			<form method="post">
				<input type="hidden" name="action" value="bulkUpdateAssigned" />
            <div id="tabs">
                <ul>
                    <li><a href="#tabs-1">Features (<cfoutput>#featureProperties.RecordCount#</cfoutput>)</a></li>
                    <li><a href="#tabs-2">Specifications (<cfoutput>#specProperties.RecordCount#</cfoutput>)</a></li>
                    <!---<li><a href="#tabs-3" class="alert">Unassigned (<cfoutput>#strayProperties.RecordCount#</cfoutput>)</a></li>--->
                </ul>
                <div id="tabs-1">
                    <!--- display feature properties --->
                    <cfset featurePropertiesDisplay = application.view.PropertyManager.getPropertyTable(featureProperties,false)>
                    <cfoutput>#featurePropertiesDisplay#</cfoutput>
                </div>
                <div id="tabs-2">
                    <!--- display spec properties --->
                    <cfset specPropertiesDisplay = application.view.PropertyManager.getPropertyTable(specProperties,false)>
