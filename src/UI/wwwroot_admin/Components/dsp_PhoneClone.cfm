...
<cfif IsDefined("form.action")>
	...
	<cfif form.action eq "clonePhone">
		<cfset newPhoneGuid = application.model.AdminPhone.clonePhone(request.p.productGuid, request.p.deviceUPC, request.p.accessoryGersSku, request.p.channelId, request.p.active, request.p.productId) />
		<cfset message = "Phone has been channelized" />
...
<div>
 	<cfset clonePhoneFormDisplay = application.view.AdminPhone.getClonePhoneForm(productGuid) />
