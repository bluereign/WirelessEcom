...
<cfif IsDefined("form.action")>
	...
	<cfif form.action eq "clonePlan">
		...
		<cfset newPlanGuid = application.model.AdminPlan.cloneMasterPlan(request.p.productGuid, request.p.name, request.p.newUPC, request.p.oldSku, request.p.accessoryGersSku, request.p.channelId, request.p.active, request.p.productId) />
...
<div>
 	<cfset clonePlanFormDisplay = application.view.AdminPlan.getMasterClonePlanForm(productGuid) />
