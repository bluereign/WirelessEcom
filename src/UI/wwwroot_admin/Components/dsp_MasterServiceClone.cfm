...
<cfif IsDefined("form.action")>
	...
	<cfif form.action eq "cloneService">
		<cfset newServiceGuid = application.model.ServiceManager.cloneMasterService(request.p.serviceGuid, request.p.name, request.p.newUPC, request.p.oldSku, request.p.accessoryGersSku, request.p.channelId, request.p.active, request.p.productId) />
...
<div>
 	<cfset cloneServiceFormDisplay = application.view.ServiceManager.getMasterCloneServiceForm(productGuid) />
