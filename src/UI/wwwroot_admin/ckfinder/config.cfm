...
//local server physical path settings
application.config.configPath = getDirectoryFromPath(getCurrentTemplatePath());
application.pathDelim = '\';
if(application.config.configPath contains '/'){
	application.pathDelim = '/';
}
//both lines are needed to clean the path
application.config.configPath = ListDeleteAt(application.config.configPath,ListLen(application.config.configPath,application.pathDelim),application.pathDelim);
application.config.configPath = ListDeleteAt(application.config.configPath,ListLen(application.config.configPath,application.pathDelim),application.pathDelim);

application.config.pathDelim = application.pathDelim;
application.config.CMSSOURCEIMAGESFOLDER = application.config.webDocRoot & "marketing" & application.config.pathDelim & "assets" & application.config.pathDelim ;
application.config.CKFINDER.baseURL = "/marketing/assets/";
...
config.baseUrl = application.config.CKFINDER.baseURL;
...
//ATTENTION: The trailing slash is required.
config.baseDir = application.config.CMSSourceImagesFolder;
