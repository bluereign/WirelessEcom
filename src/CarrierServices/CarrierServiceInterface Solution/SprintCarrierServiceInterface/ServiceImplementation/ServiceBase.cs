...
namespace SprintCSI.ServiceImplementation
{
    ...
    public class ServiceBase  
    {
        ...
        {
            var certFiles = Directory.GetFiles(HttpContext.Current.Server.MapPath("App_Data"));
            foreach (var s in certFiles)
            {
                if (s.EndsWith("cer"))
                {
        ...
        {
            new Log().LogException(exception, "Sprint", requestType, this.referenceNumber);
        }
        ...
        {
            new Log().LogRequest(new Utility().SerializeXML(objectToLog), "Sprint", requestType, this.referenceNumber);
        }
        ...
        {
            new Log().LogResponse(new Utility().SerializeXML(objectToLog), "Sprint", requestType, this.referenceNumber);
        }
