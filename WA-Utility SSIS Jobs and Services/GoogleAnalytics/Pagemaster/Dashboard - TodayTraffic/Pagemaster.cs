    ...
    class Program
    {
       ...
       static string GetInputDateString(string[] args)
       {
        ...
        static void Main(string[] args)
        {
            ...
            VarCharFeed.Dimensions = "ga:hostname,ga:source";
            VarCharFeed.Metrics = "ga:visits,ga:visitors,ga:bounces,ga:avgTimeOnSite,ga:pageviewsPerVisit";
            ...
            string sql = "DELETE FROM dashboard.Analytics WHERE RunDate = '" + date + "'";
            ...
            for (int i = 0; i < feed.Entries.Count; i++)
            {
                ...
                string hostname = pvEntry.Dimensions[0].Value.Substring(0);
                string source = pvEntry.Dimensions[1].Value.Substring(0);
                string visits = pvEntry.Metrics[0].Value;
                string visitors = pvEntry.Metrics[1].Value;
                string bounces = pvEntry.Metrics[2].Value;
                string avgtime = pvEntry.Metrics[3].Value;
                string avgpages = pvEntry.Metrics[4].Value;
                ...
                string sql2 = "INSERT INTO dashboard.Analytics " +
                        " (RunDate, Hostname, Source,VisitsTotal,VisitsUnique,Bounces,AvgTime,AvgPages) " +
                        " VALUES ('" + date + "','" + hostname + "','" + source + "','" + visits + "','" + visitors + "','" + bounces + "','" + avgtime + "','" + avgpages + "')";
