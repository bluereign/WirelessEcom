    ...
    class Program
    {
       ...
       static string GetInputDateString(string[] args)
       {
        ...
        static void Main(string[] args)
        {
            ...
            VarCharFeed.Dimensions = "ga:searchKeyword";
            VarCharFeed.Metrics = "ga:searchUniques,ga:searchResultViews,ga:searchRefinements,ga:searchDepth,ga:transactions,ga:searchExits";
            //VarCharFeed.Metrics = "ga:searchUniques";
            VarCharFeed.Sort = "-ga:searchUniques";
            ...
            string conn = "Data Source=10.7.0.220;Initial Catalog=AnalyticsReporting;Integrated Security=True;Persist Security Info=False;";

            SqlConnection sc = new SqlConnection(conn);
            sc.Open();

            string sql = "DELETE FROM dashboard.Search WHERE RunDate = '" + date + "'";
            ...
            //foreach (DataEntry pvEntry in feed.Entries)
            {
                ...
                string term = pvEntry.Dimensions[0].Value.Substring(0);
                string unique = pvEntry.Metrics[0].Value;
                string results = pvEntry.Metrics[1].Value;
                string refined = pvEntry.Metrics[2].Value;
                string depth = pvEntry.Metrics[3].Value;
                string transactions = pvEntry.Metrics[4].Value;
                string exits = pvEntry.Metrics[5].Value;


            string conn2 = "Data Source=10.7.0.220;Initial Catalog=AnalyticsReporting;Integrated Security=True;Persist Security Info=False;";
            
            SqlConnection sc2 = new SqlConnection(conn2);
            sc2.Open();


            string sql2 = "INSERT INTO dashboard.Search " +
                    " (RunDate, SearchTerm, UniqueSearches, TotalResults, TermRefined, Depth, Transactions, Exits) " +
                    //" (RunDate, SearchTerm, UniqueSearches) " +
                    " VALUES ('" + date + "','" + term.Replace("'","''") + "','" + unique + "','" + results + "','" + refined + "','" + depth + "','" + transactions + "','" + exits + "')";
